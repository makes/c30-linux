...
static
#endif
void add_lineno PARAMS ((fragS *, addressT, int));
...
#if defined(TC_PIC30)
    /*
     * Only set the listing hll file for the main file.
     * This change is done in support of annotating dependencies
     * in the COFF file.
     */
    static  symbolS *lMainFileSymbolP;
    if (lMainFileSymbolP == NULL)
    {
      lMainFileSymbolP = symbolP;
      if (listing)
      {
     ...
     int ignore ATTRIBUTE_UNUSED;
{
  ...
  /* If there is no lineno symbol, treat a .loc directive as if it were a
     .appline directive.  */
...
#if defined(TC_PIC30)
	S_SET_SEGMENT (def_symbol_in_progress, pic30_last_code_section);
#else
...
#if defined (TC_PIC30)
  if (section_name == input_line_pointer) {
  ...
  if (*input_line_pointer == ',')
      ...
      else
	{
	  ...
	  while (*input_line_pointer != '"'
		...
		case 'd': flags |= SEC_DATA | SEC_LOAD | SEC_ALLOC; /* fall through */
		case 'w': flags &=~ SEC_READONLY; break;
#else
		case 'w':
		case 'd': flags |= SEC_DATA | SEC_LOAD | SEC_ALLOC; break;
#endif
		case 'a': break; /* For compatability with ELF.  */
		case 'x': flags |= SEC_CODE | SEC_LOAD | SEC_ALLOC; break;
		case 'r': flags |= SEC_READONLY | SEC_ALLOC; break;
...
#if defined(TC_PIC30)
  /* set attributes not fully represented by the flags word */
  if ((flags & SEC_READONLY) == SEC_READONLY)
...
#else
  bfd_vma align_power = ((PIC30_IS_REVERSE_ATTR(sec) |
                          PIC30_IS_UNORDERED_ATTR(sec)) ?
                         1 : (bfd_vma)sec->alignment_power);
#endif
