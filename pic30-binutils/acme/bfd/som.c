     ...
     unsigned long current_offset;
{
  ...
  for (section = abfd->sections; section; section = section->next)
    {
      if (((section->flags & SEC_CODE) == 0) || (section->auxflash == 0))
	continue;
	  ...
	  if (first_subspace
	      && (abfd->flags & (EXEC_P | DYNAMIC)))
	    {
		 ...
		 within the file regardless of the executable's type.  */
		  ...
		  || (subsection->auxflash == 1)
		  || ((abfd->flags & WP_TEXT)
		      && (subsection->flags & SEC_DATA)))
		current_offset = SOM_ALIGN (current_offset, PA_PAGESIZE);

	      /* Update the exec header.  */
	      if (((subsection->flags & SEC_CODE) ||                                               (subsection->auxflash == 1)) &&                                                exec_header->exec_tfile == 0)
		{
	  ...
	  else if (abfd->flags & (EXEC_P | DYNAMIC))
	    {
		 ...
		 ps.  This is not necessary for unloadable space/subspaces.  */
	      ...
	      if ((subsection->flags & SEC_CODE) || (subsection->auxflash == 1))
     ...
     struct som_misc_symbol_info *info;
{
  ...
  if (sym->flags & BSF_SECTION_SYM)
	...
	{
	  if ((sym->section->flags & SEC_CODE) || (sym->section->auxflash == 1))
	    info->symbol_type = ST_CODE;
	  else
	    info->symbol_type = ST_DATA;
	}
