...
typedef struct disassemble_info {
  ...
  /* since not all targets have a unified memory archtiecture, it is sometimes
     necessary for section flags to match a particular value when searching
     for symbols.... if this is the case for YOUR architecture, you can set
     flags_to_match to a section type first:
        currently symbols are sorted by SEC_CODE, SEC_DATA, SEC_READONLY (in
        that order).  Have a safe and wonderful day. */
...
extern int pic30_print_insn		PARAMS ((bfd_vma, disassemble_info*));
...
extern void pic30_print_disassembler_options PARAMS ((FILE *));
extern void pic30_parse_disassembler_option  PARAMS ((char *));
